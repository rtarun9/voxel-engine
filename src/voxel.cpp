...
{
    other.m_color_buffer.m_resource.Reset();
    other.m_index_buffer.m_resource.Reset();
            ...
            std::pair{renderer.m_copy_queue.m_monotonic_fence_value + 1,
                      m_thread_pool.add_to_task_queue([this, &renderer, chunk_position = top]() {
                          ...
                          if (!chunk_index_data.empty())
                          {
                              ...
                              setup_chunk_data.m_chunk_index_buffer = renderer.create_index_buffer(
                                  (void *)chunk_index_data.data(), sizeof(u16), chunk_index_data.size(),
                                  std::wstring(L"Chunk Index buffer : ") + chunk_position.to_wstring());
                              setup_chunk_data.m_chunk_color_buffer = renderer.create_structured_buffer(
                                  (void *)color_data.data(), sizeof(DirectX::XMFLOAT3), color_data.size(),
                                  std::wstring(L"Chunk color buffer : ") + chunk_position.to_wstring());
                          }
